const DefaultConfig = {
  "logoText": "ABI-Tech",
  "panelBlur": 5,
  "adminRules": [
  {
    "amount": 203999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 3000
  },
  {
    "amount": 203999,
    "feeType": "flat",
    "keyword": "TF, EDC",
    "feeValue": 3000
  },
  {
    "amount": 203999,
    "feeType": "flat",
    "keyword": "TIKET, Auto Deposit",
    "feeValue": 3000
  },
  {
    "amount": 505999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 5000
  },
  {
    "amount": 1010999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 10000
  },
  {
    "amount": 1515999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 15000
  },
  {
    "amount": 2020999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 20000
  },
  {
    "amount": 2525999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 25000
  },
  {
    "amount": 3030999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 30000
  },
  {
    "amount": 3535999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 35000
  },
  {
    "amount": 4040999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 40000
  },
  {
    "amount": 4545999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 45000
  },
  {
    "amount": 5005999,
    "feeType": "flat",
    "keyword": "TF, EDC",
    "feeValue": 5000
  },
  {
    "amount": 5005999,
    "feeType": "flat",
    "keyword": "TIKET, Auto Deposit",
    "feeValue": 5000
  },
  {
    "amount": 5050999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 50000
  },
  {
    "amount": 5555999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 55000
  },
  {
    "amount": 6060999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 60000
  },
  {
    "amount": 6565999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 65000
  },
  {
    "amount": 7070999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 70000
  },
  {
    "amount": 7575999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 75000
  },
  {
    "amount": 8080999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 80000
  },
  {
    "amount": 8585999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 85000
  },
  {
    "amount": 9090999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 90000
  },
  {
    "amount": 9595999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 95000
  },
  {
    "amount": 10010999,
    "feeType": "flat",
    "keyword": "TF, EDC",
    "feeValue": 10000
  },
  {
    "amount": 10010999,
    "feeType": "flat",
    "keyword": "TIKET, Auto Deposit",
    "feeValue": 10000
  },
  {
    "amount": 10100999,
    "feeType": "flat",
    "keyword": "QR",
    "feeValue": 100000
  },
  {
    "amount": 20020999,
    "feeType": "flat",
    "keyword": "TF, EDC",
    "feeValue": 20000
  },
  {
    "amount": 20020999,
    "feeType": "flat",
    "keyword": "TIKET, Auto Deposit",
    "feeValue": 20000
  },
  {
    "amount": 50025999,
    "feeType": "flat",
    "keyword": "TF, EDC",
    "feeValue": 25000
  },
  {
    "amount": 50025999,
    "feeType": "flat",
    "keyword": "TIKET, Auto Deposit",
    "feeValue": 25000
  }
],
  "auditRules": [
    {
      "keyword1": "REV TARTUN QR",
      "keyword2": "TARTUN QR"
    },
    {
      "keyword1": "REV TARTUN TF",
      "keyword2": "TARTUN TF"
    },
    {
      "keyword1": "REV TARTUN EDC",
      "keyword2": "TARTUN EDC"
    },
    {
      "keyword1": "REVISI TARTUN QR",
      "keyword2": "TARTUN QR"
    },
    {
      "keyword1": "REVISI TARTUN TF",
      "keyword2": "TARTUN TF"
    },
    {
      "keyword1": "REVISI TARTUN EDC",
      "keyword2": "TARTUN EDC"
    },
    {
      "keyword1": "REV",
      "keyword2": ""
    }
  ],
  "isFlatTheme": true,
  "monthEndDay": 28,
  "panelOpacity": 0.8,
  "backgroundUrl": "https://4kwallpapers.com/images/wallpapers/lisa-blackpink-thai-singer-asian-girl-k-pop-singer-3840x2160-3232.jpg",
  "monthStartDay": 29,
  "chartDataLimit": 50,
  "logoDescription": "Laporan Tarik Tunai Outlet",
  "routingKeywords": {
    "tiket": [
      "tiket deposit",
      "Auto Deposit"
    ],
    "manual": [
      "tar",
      "tartun",
      "tf",
      "qr",
      "EDC"
    ]
  },
  "announcementText": "JANGAN PERNAH NGASIH UANG DULU SEBELUM TIKET SETTLED ATAU TF MASUK.",
  "targetCommission": 15000000,
  "whatsappContacts": [
    {
      "name": "CS-AbiTech",
      "number": "6281394011006"
    }
  ],
  "adminBankKeywords": [
    "QR"
  ],
  "announcementStyle": {
    "color": "warning",
    "fontSize": "xl",
    "animation": "bounce",
    "fontWeight": "bold"
  },
  "auditPanelEnabled": true,
  "exceptionKeywords": [
    "ADM TARTUN",
    "ADMIN TARTUN",
    "BAYAR",
    "SETOR",
    "Keterangan"
  ],
  "nameConsolidation": {
    "PLC CK": "PARENT CIKADUT",
    "PLC DM": "PARENT DM",
    "PLC RK": "PARENT RK",
    "PLC SA": "PARENT SUKAASIH",
    "PLC SS": "PARENT SS",
    "PLC JH2": "PARENT JH2",
    "PLC PD1": "PARENT PD1",
    "PLC PD2": "PARENT PADASUKA 2",
    "PLC PD3": "PARENT PD3",
    "PLC CISA": "PARENT CISAR",
    "PLC CPD1": "PARENT CIPADUNG",
    "PLC CPD2": "PARENT CIPADUNG2",
    "PLC CIGER": "PARENT CIGER",
    "PLC SINOM": "PARENT BK SINOM",
    "PLC ASBER1": "PARENT ASBER 1",
    "PLC ASBER2": "PARENT ASBER 2",
    "PLC CUKANG": "PARENT CICUKANG",
    "PLC PC3 CJM": "PARENT CJM PC3",
    "PLC BUNISARI": "PARENT BUNISARI",
    "PLC PC4 OJEG": "PARENT OJEG PC4",
    "PLC BK PORTAL": "PARENT PORTAL CJM",
    "PLC CIKADUT 2": "PARENT CIKADUT 2",
    "PLC BK8 BAKSAR": "PARENT BK8 BAKSAR",
    "PLC BK9 BAKSAR": "PARENT BK9 BAKSAR",
    "PLC PANGARITAN": "PARENT PANGARITAN",
    "PLC BK7 NAGROG2": "PARENT NAGROG2 BK7",
    "PLC ALFA2 SINJAY2": "PARENT SINJAY2 ALFA 2",
    "PLC ALFA3 SINJAY1": "PARENT SINJAY1 ALFA 3",
    "PLC ALFA4 PARAKAN": "PARENT PARAKANSAAT ALFA4",
    "PLC ALFA5 NAGROG1": "PARENT NAGROG1 ALFA 5",
    "PLC CILENGKRANG 1": "PARENT CILENGKRANG 1",
    "PLC CIPAGALO CELL": "PARENT CIPAGALO",
    "PLC PC5 CIGENDING": "PARENT CIGENDING PC5",
    "PLC ALFA6 CINANGKA": "PARENT CINANGKA ALFA 6",
    "PLC ALFA7 CINGISED": "PARENT ALFA7 CINGISED",
    "PLC ALFA1 PASIR IMPUN": "PARENT PASIRIMPUN ALFA1",
    "ID1026574479725": "PARENT PASIRIMPUN ALFA1",
    "ALFA 1 CELL": "PARENT PASIRIMPUN ALFA1",
    "ID1026574479691": "PARENT SINJAY2 ALFA 2",
    "ALFA 2 CELL": "PARENT SINJAY2 ALFA 2",
    "ID1026574479766": "PARENT SINJAY1 ALFA 3",
    "ALFA 3 CELL": "PARENT SINJAY1 ALFA 3",
    "ID1026574479709": "PARENT PARAKANSAAT ALFA4",
    "ALFA 4 CELL": "PARENT PARAKANSAAT ALFA4",
    "ID1026574479741": "PARENT NAGROG1 ALFA 5",
    "ALFA 5 CELL": "PARENT NAGROG1 ALFA 5",
    "ID1026574478578": "PARENT CINANGKA ALFA 6",
    "ALFA 6 CELL": "PARENT CINANGKA ALFA 6",
    "ID1026574478586": "PARENT ALFA7 CINGISED",
    "ALFA 7 CELL": "PARENT ALFA7 CINGISED",
    "ID1026574478594": "PARENT ASBER 1",
    "ASBER 1 CELL": "PARENT ASBER 1",
    "ID1026574478560": "PARENT ASBER 2",
    "ASBER 2 CELL": "PARENT ASBER 2",
    "ID1026575135805": "PARENT BK8 BAKSAR",
    "BAKSAR 1 CELL": "PARENT BK8 BAKSAR",
    "ID1026574492439": "PARENT BK9 BAKSAR",
    "BAKSAR 2 CELL": "PARENT BK9 BAKSAR",
    "ID1022223873046": "BANDAR KUOTA QR",
    "BANDAR KUOTA QR": "BANDAR KUOTA QR",
    "ID1026574478552": "PARENT CIGER",
    "BK 5 CIGER CELL": "PARENT CIGER",
    "ID1026575135789": "PARENT PANGARITAN",
    "BK 6 PANGARITAN CELL": "PARENT PANGARITAN",
    "ID1026575135821": "PARENT NAGROG2 BK7",
    "BK 7 NAGROG CELL": "PARENT NAGROG2 BK7",
    "ID1026574492447": "PARENT PORTAL CJM",
    "BK CIJAMBE CELL": "PARENT PORTAL CJM",
    "ID1026575060516": "PARENT CIPADUNG",
    "BK CIPADUNG CELL": "PARENT CIPADUNG",
    "ID1026574492462": "PARENT JH2",
    "BK JH 2 CELL": "PARENT JH2",
    "ID1026574492470": "PARENT BK SINOM",
    "BK SINOM CELL": "PARENT BK SINOM",
    "ID1026570614358": "PARENT BUNISARI",
    "BUNISARI CELL": "PARENT BUNISARI",
    "ID1026574492454": "PARENT CICUKANG",
    "CICUKANG CELL": "PARENT CICUKANG",
    "ID1026574480152": "PARENT CIHAURKUKU",
    "CIHAURKUKU CELL": "PARENT CIHAURKUKU",
    "PLC CIHAURKUKU": "PARENT CIHAURKUKU",
    "ID1026575042621": "PARENT CIKADUT 2",
    "CIKADUT 2 CELL": "PARENT CIKADUT 2",
    "ID1026575042639": "PARENT CIKADUT",
    "CIKADUT CELL": "PARENT CIKADUT",
    "PLC CIKADUT 1": "PARENT CIKADUT",
    "ID1026575042647": "PARENT CILENGKRANG 1",
    "CILENGKRANG 1 CELL": "PARENT CILENGKRANG 1",
    "PLC CL 1": "PARENT CILENGKRANG 1",
    "ID1026575042654": "PARENT CILENGKRANG 2",
    "CILENGKRANG 2 CELL": "PARENT CILENGKRANG 2",
    "PLC CL 2": "PARENT CILENGKRANG 2",
    "ID1026575042613": "PARENT CILENGKRANG 3",
    "CILENGKRANG 3 CELL": "PARENT CILENGKRANG 3",
    "PLC CL 3": "PARENT CILENGKRANG 3",
    "ID1026575060524": "PARENT CILENGKRANG 4",
    "CILENGKRANG 4 CELL": "PARENT CILENGKRANG 4",
    "PLC CL 4": "PARENT CILENGKRANG 4",
    "ID1026575060557": "PARENT CIPADUNG2",
    "CIPADUNG 2 CELL": "PARENT CIPADUNG2",
    "PLC CIPADUNG 2": "PARENT CIPADUNG2",
    "PLC BK CIPADUNG": "PARENT CIPADUNG",
    "PLC BK CIJAMBE": "PARENT PORTAL CJM",
    "PLC BK5 CIGER": "PARENT CIGER",
    "PLC BK6 PANGARITAN": "PARENT PANGARITAN",
    "PLC BAKSAR1": "PARENT BK8 BAKSAR",
    "PLC BAKSAR2": "PARENT BK9 BAKSAR",
    "ID1026574487421": "PARENT CIPAGALO",
    "CIPAGALO CELL": "PARENT CIPAGALO",
    "ID1026575060532": "PARENT CIPOREAT",
    "CIPOREAT CELL": "PARENT CIPOREAT",
    "PLC CIPOREAT": "PARENT CIPOREAT",
    "ID1026575060540": "PARENT CISAR",
    "CISARANTEN CELL": "PARENT CISAR",
    "ID1026574486258": "PARENT DM",
    "DM CELL": "PARENT DM",
    "ID1026574486274": "PARENT PD1",
    "PADASUKA 1 CELL": "PARENT PD1",
    "ID1026574486225": "PARENT PADASUKA 2",
    "PADASUKA 2 CELL": "PARENT PADASUKA 2",
    "ID1026574486241": "PARENT PD3",
    "PADASUKA 3 CELL": "PARENT PD3",
    "ID1026574486233": "PARENT CJM PC3",
    "PC 3 CELL": "PARENT CJM PC3",
    "ID1026574487462": "PARENT OJEG PC4",
    "PC 4 CELL": "PARENT OJEG PC4",
    "ID1026574487439": "PARENT CIGENDING PC5",
    "PC 5 CELL": "PARENT CIGENDING PC5",
    "ID1026574480095": "PARENT PERMATA",
    "PERMATA CELL": "PARENT PERMATA",
    "KBB PERMATA": "PARENT PERMATA",
    "PLC PERMATA": "PARENT PERMATA",
    "ID1022225940488": "BANDAR KUOTA QR",
    "POLICE CELL I QR": "BANDAR KUOTA QR",
    "POLICE RELOAD QR": "BANDAR KUOTA QR",
    "ID1026574480137": "PARENT RAWA 2",
    "RAWA CELL": "PARENT RAWA 2",
    "KBB RAWA 2": "PARENT RAWA 2",
    "PLC RAWA 2": "PARENT RAWA 2",
    "PLC RAWA": "PARENT RAWA 2",
    "ID1026574480145": "PARENT RK",
    "RK CELL": "PARENT RK",
    "ID1026574487454": "PARENT SUKAASIH",
    "SA CELL": "PARENT SUKAASIH",
    "ID1026574487447": "PARENT SS",
    "SS CELL": "PARENT SS"
  },
  "adminBankFeePercent": 0,
  "dataParsingSettings": {
    "columnOrder": [
      "tanggal",
      "nama",
      "jumlah",
      "keterangan"
    ],
    "dateFormats": [
      {
        "id": "iso_8601",
        "label": "ISO 8601 (Sangat Detail)",
        "active": true,
        "format": "YYYY-MM-DDTHH:mm:ss.sssZ",
        "example": "2025-07-06T21:04:27.907Z"
      },
      {
        "id": "yyyy_mm_dd",
        "label": "Tahun-Bulan-Hari",
        "active": true,
        "format": "YYYY-MM-DD",
        "example": "2025-07-20"
      },
      {
        "id": "dd_mm_yyyy",
        "label": "Hari/Bulan/Tahun",
        "active": true,
        "format": "DD/MM/YYYY",
        "example": "20/07/2025"
      },
      {
        "id": "mm_dd_yyyy",
        "label": "Bulan/Hari/Tahun",
        "active": false,
        "format": "MM/DD/YYYY",
        "example": "07/20/2025"
      },
      {
        "id": "dd_mmm_yyyy",
        "label": "Hari-Bln-Tahun (Singkat)",
        "active": false,
        "format": "DD-MMM-YYYY",
        "example": "20-Jul-2025"
      },
      {
        "id": "dd_mmmm_yyyy",
        "label": "Hari Bulan Tahun (Lengkap)",
        "active": false,
        "format": "DD MMMM YYYY",
        "example": "20 Juli 2025"
      }
    ],
    "csvDelimiter": ";",
    "pasteDelimiter": "\\t"
  },
  "publicSummaryColumns": [
    {
      "id": "namaPengguna",
      "align": "text-left",
      "label": "Nama Pengguna",
      "visible": true,
      "isCurrency": false
    },
    {
      "id": "manualFee",
      "align": "text-right",
      "label": "Admin",
      "visible": true,
      "isCurrency": true
    },
    {
      "id": "tiketFee",
      "align": "text-right",
      "label": "Nom. Tiket",
      "visible": true,
      "isCurrency": true
    },
    {
      "id": "totalAdminFee",
      "align": "text-right",
      "label": "Total Biaya Admin",
      "visible": true,
      "isCurrency": true
    },
    {
      "id": "commissionOutlet",
      "align": "text-right",
      "label": "Komisi Outlet",
      "visible": true,
      "isCurrency": true
    },
    {
      "id": "commissionCS",
      "align": "text-right",
      "label": "Komisi CS",
      "visible": false,
      "isCurrency": true
    },
    {
      "id": "count",
      "align": "text-right",
      "label": "Transaksi",
      "visible": true,
      "isCurrency": false
    }
  ],
  "ticketFeeDestination": "adminFee",
  "publicDashboardLayout": [
    {
      "id": "announcement",
      "size": "full",
      "label": "Pengumuman",
      "visible": true
    },
    {
      "id": "kpiMonthCommission",
      "size": "small",
      "label": "Total Komisi (Bulan Ini)",
      "visible": true
    },
    {
      "id": "kpiYesterdayTotal",
      "size": "small",
      "label": "Total Transaksi (Kemarin)",
      "visible": true
    },
    {
      "id": "kpiTodayTotal",
      "size": "small",
      "label": "Total Transaksi (Hari Ini)",
      "visible": true
    },
    {
      "id": "activeOutletsCount",
      "size": "small",
      "label": "Outlet Aktif (Bulan Ini)",
      "visible": true
    },
    {
      "id": "kpiYesterdayCount",
      "size": "small",
      "label": "Jumlah Transaksi (Kemarin)",
      "visible": true
    },
    {
      "id": "kpiTodayCount",
      "size": "small",
      "label": "Jumlah Transaksi (Hari Ini)",
      "visible": true
    },
    {
      "id": "progressCommission",
      "size": "full",
      "label": "Progress Target Komisi",
      "visible": true
    },
    {
      "id": "trendChart",
      "size": "full",
      "label": "Trend Biaya Admin (7 Hari)",
      "visible": true
    },
    {
      "id": "tableTopOutlets",
      "size": "small",
      "label": "Top 5 Outlet (Komisi Bulan Ini)",
      "visible": true
    },
    {
      "id": "chartTxType",
      "size": "small",
      "label": "Komposisi Tipe Transaksi (Bulan Ini)",
      "visible": true
    },
    {
      "id": "utilAdminCalculator",
      "size": "small",
      "label": "Kalkulator Biaya Admin",
      "visible": true
    },
    {
      "id": "globalCommissionSummary",
      "size": "small",
      "label": "Ringkasan Global (Bulan Ini)",
      "visible": false
    },
    {
      "id": "kpiMonthTopUser",
      "size": "half",
      "label": "Outlet Teraktif (Bulan Ini)",
      "visible": false
    },
    {
      "id": "tableRecentTx",
      "size": "half",
      "label": "5 Transaksi Terkini",
      "visible": false
    }
  ],
  "csCommissionPercentage": 10,
  "outletCommissionPercentage": 20,
  "dashboardWidgets": [
    {
      "id": "announcement",
      "size": "full",
      "label": "Pengumuman",
      "visible": false
    },
    {
      "id": "globalCommissionSummary",
      "size": "small",
      "label": "Ringkasan Global (Bulan Ini)",
      "visible": true
    },
    {
      "id": "kpiMonthTopUser",
      "size": "small",
      "label": "Outlet Teraktif (Bulan Ini)",
      "visible": false
    },
    {
      "id": "kpiMonthCommission",
      "size": "small",
      "label": "Total Komisi (Bulan Ini)",
      "visible": false
    },
    {
      "id": "kpiYesterdayTotal",
      "size": "small",
      "label": "Total Transaksi (Kemarin)",
      "visible": true
    },
    {
      "id": "kpiTodayTotal",
      "size": "small",
      "label": "Total Transaksi (Hari Ini)",
      "visible": true
    },
    {
      "id": "activeOutletsCount",
      "size": "small",
      "label": "Outlet Aktif (Bulan Ini)",
      "visible": true
    },
    {
      "id": "kpiTodayCount",
      "size": "small",
      "label": "Jumlah Transaksi (Hari Ini)",
      "visible": true
    },
    {
      "id": "kpiYesterdayCount",
      "size": "small",
      "label": "Jumlah Transaksi (Kemarin)",
      "visible": true
    },
    {
      "id": "progressCommission",
      "size": "full",
      "label": "Progress Target Komisi",
      "visible": true
    },
    {
      "id": "tableTopOutlets",
      "size": "small",
      "label": "Top 5 Outlet (Komisi Bulan Ini)",
      "visible": true
    },
    {
      "id": "chartTxType",
      "size": "small",
      "label": "Komposisi Tipe Transaksi (Bulan Ini)",
      "visible": true
    },
    {
      "id": "tableRecentTx",
      "size": "small",
      "label": "5 Transaksi Terkini",
      "visible": true
    },
    {
      "id": "trendChart",
      "size": "full",
      "label": "Trend Biaya Admin (7 Hari)",
      "visible": true
    },
    {
      "id": "utilAdminCalculator",
      "size": "small",
      "label": "Kalkulator Biaya Admin",
      "visible": false
    }
  ]
};

const AppState = {
    allData: [],
    settings: {},
    defaultConfig: DefaultConfig,
    chartInstances: {},
    currentUser: null,
    dataChannel: null,
    logChannel: null,
    activeView: 'dashboard',
    lastSummaryData: [],
    isAuditMode: false,
    isAllDataMode: false,
    analysisSort: {
        column: 'tanggal',
        direction: 'desc'
    },
    // --- PENAMBAHAN STATE UNTUK PENGURUTAN RINGKASAN ---
    summarySort: {
        column: 'commissionOutlet', // Kolom default untuk diurutkan
        direction: 'desc' // Arah default (menurun)
    },
    // --- AKHIR PENAMBAHAN ---
    analysisColumnFilters: {},
    analysisUiState: {
        focusedColumn: null,
        cursorPosition: null
    },
    stagingData: [],
    lastImportBatchId: localStorage.getItem('fkof_lastImportBatchId') || null,
    isInitialAuthCheckComplete: false,
    isProcessingAuthChange: false,
    virtualScrollInstances: {},
    analysisSelectedIds: new Set(),
    modalSelectedIds: new Set(),
    currentAuditPairs: [],
    filterCache: new Map(),
    dataIndexes: {},
    filterPresets: [], 
    isFilterPanelVisible: true,
    sessionCheckInterval: null,
};


